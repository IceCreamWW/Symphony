$(function () {
    ...
    $('.route-name-editable-wrapper input').blur(function (event) {
        ...
        $.post('modify_route_name', {id: mExtMap.routes.getCurRoute().id, name: newName}, function (data) {
            ...
            $('#profile_route_ul').trigger("change-name", [mExtMap.routes.getCurRoute().id, newName])
        });
    ...
    $placesList.on(
        {
            ...
            'add-place': function (event, markerId) {
                ...
                newPlaceElement.find('.place-name').attr('title', marker.name);
                /* 补全元素事件 */
            ...
            'refresh': function (event, forceRefresh) {
                ...
                $('.places-tool-share').find('a').attr('href', genSNShref(mExtMap.routes.getCurRoute().name, mExtMap.routes.getCurRoute().id));
